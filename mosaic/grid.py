...
logger = logging.getLogger(
    __name__
)  # the __name__ resolve to "grid" since we are at the root of the project.
...
class Grid:
    def __init__(self, diamond=True, colorful=True, progress_callback=None):
    ...
    def setup(self, imgpath, pix=0, pix_multi=-1, working_res=0, enlarge=0):
        data = Image.open(imgpath)  # <-- may raise ...
        ...
        if working_res:
            if (
                working_res < self.og_image.size[0]
                and working_res < self.og_image.size[1]
            ):
                self.og_image = util.restrain_img_size(
                    self.og_image, max_pix=working_res
                )
            else:
                self.og_image = util.enlarge_img(self.og_image, max_pix=working_res)

        logger.info(f"working_res: {working_res}")

        self.N = util.get_multi(self.og_image, self.enlarge * self.M)
        ...
        if self.is_diamond:
            self.og_size = (
                self.og_image.size[0] * self.N,
                self.og_image.size[1] * self.N,
            )
            self.og_image = self.og_image.rotate(
                45, expand=True, resample=Image.BICUBIC
            )
        else:
        ...
        logger.info(f"og_size: {self.og_size}")

        # Find edges
        # if (use_skimage):
        #    self.image_array = np.array(self.edg_img)
        #    self.img_edges = feature.canny(rgb2gray(self.image_array), sigma=2)
        img_gray = self.edg_img.convert("L")
        self.img_edges = np.array(img_gray.filter(ImageFilter.FIND_EDGES))
        ...
        self.grid_status = np.zeros(
            [self.width // self.pixels, self.height // self.pixels]
        )
    ...
    def occupy(self, x, y, x_total=1, y_total=1):
        for i in range(x_total):
            for j in range(y_total):
                if (
                    x + i < self.width // self.pixels
                    and y + j < self.height // self.pixels
                ):
    ...
    def best_shape(self, cropped_img):
        ...
        circle, circle_rms = CircleCell.find_best(
            cropped_img,
            base_colors=base_colors_3,
            second_colors=second_colors_2,
            N=self.N,
        )
        rect, rect_rms = RectCell.find_best(
            cropped_img,
            base_colors=base_colors_2,
            second_colors=second_colors_2,
            N=self.N,
        )
        pie, pie_rms = PieSliceCell.find_best(
            cropped_img,
            base_colors=base_colors_3,
            second_colors=second_colors_2,
            N=self.N,
        )
        halfc, halfc_rms = HalfCircleCell.find_best(
            cropped_img,
            base_colors=base_colors_3,
            second_colors=second_colors_2,
            N=self.N,
        )
        triangle, triangle_rms = TriangleCell.find_best(
            cropped_img,
    ...
    def grid_start_end(self, s_row, f_row):
                ...
                if not self.is_occupied(col, row):
                    ...
                    if np.any(edges_seg) and len(np.where(edges_seg)[1]):
                        ...
                        if randint(0, 2):  # 0 or 1
                            ...
                            rect_coords2[3] = util.clamp_int(
                                rect_coords2[3] + pix, 0, height
                            )
                            pix_h *= 2
                        else:
                            """
                            ...
                            rect_coords2[2] = util.clamp_int(
                                rect_coords2[2] + pix, 0, width
                            )
                            pix_w *= 2
                        ...
                        og_color = util.average_color_img(
                            self.og_image.crop(rect_coords)
                        )
                        base_colors = GenColor.gen_colors(
                            og_color, randint(3, 5), self.is_colorful
                        )
        ...
        duration_ms = util.since_ms(start_ms)
        logger.info(f"grid_start_end({s_row},{f_row}) ... {duration_ms}ms")

    def crop_grid(self, img, N=2):
        return img.crop(
            (0, 0, self.cols * self.pixels * N, self.rows * self.pixels * N)
        )
    ...
    def save(self, path, dpi=300, is_continue=False):
        self.update_progress(add=3)
        # util.print_ts('grid.save.0')

        if self.is_diamond:
            diamond_img = util.mult_img_size(self.canvas_img.copy(), 1 / self.M)
            ...
            diamond_img.save(
                path,
                "jpeg",
                icc_profile=self.og_image.info.get("icc_profile"),
                quality=95,
                dpi=(dpi, dpi),
            )
        else:
