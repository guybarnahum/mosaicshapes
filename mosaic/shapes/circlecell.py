...
class CircleCell(Cell):
    def __init__(
        self, size=(200, 200), csize=(200, 200), base_colors=[], second_colors=[]
    ):
        self.width = size[0]
        self.height = size[1]
        self.cwidth = csize[0]
        ...
        self.colors_secondary = (
            second_colors  # Cell.gen_colors(second_color,sn, colorful)
        )
    ...
    def draw(self, N=2):
        ...
        pw = int(
            round(0.2 * shortest * 1 / (len(self.colors) + len(self.colors_secondary)))
        )
